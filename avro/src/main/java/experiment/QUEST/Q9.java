...
import experiment.filter.doc.DateFilter;
import experiment.filter.doc.FeeFilter;
import experiment.filter.graph.TagFilter;
import experiment.filter.graph.UniversityFilter;
...
public class Q9 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        docFilters[0] = new FeeFilter(Float.parseFloat(args[8]), Float.parseFloat(args[9]));
        docFilters[1] = new DateFilter(args[10], args[11]);

        FilterOperator[] graphFilters = new FilterOperator[2];
        graphFilters[0] = new UniversityFilter(args[12]);
        graphFilters[1] = new TagFilter(args[13]);
        ...
        path.add(graphFilters[0].getName());
        path.add("root");
        path.add(graphFilters[1].getName());
