...
public class AvroColumnator {
    ...
    private Map<Schema, Schema> seen = new IdentityHashMap<Schema, Schema>(); //记录已经出现过的schema，防止出现递归循环
    ...
    /**
     * 判断输入的schema的类型是否为简单类型
     * @param s 需要判断的schema
     * @return 如果是列出的基本类型，返回true，否则返回false
     */
    public static boolean isSimple(Schema s) {
        switch (s.getType()) {
            case NULL:
            case BOOLEAN:
            case INT:
    ...
    /**
     * 获得schema类型对应的ValueType的类型
     * @param s 输入的schema实例
     * @return 返回s的类型对应的ValueType的类型
     */
    private ValueType simpleValueType(Schema s) {
        switch (s.getType()) {
            ...
            default:
                throw new TrevniRuntimeException("Unknown schema: " + s.getType());
        }
    }
