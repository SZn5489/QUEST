...
public class DocDataFileReader<D> extends DataFileReader implements Closeable {

    public DocDataFileReader() {
    }

    public DocDataFileReader(File file) throws IOException {
        this(file, null, GenericData.get());
    }

    public DocDataFileReader(Input data, Input head) throws IOException {
        this(data, head, null, GenericData.get());
    }

    public DocDataFileReader(File file, FilterOperator[] filters) throws IOException {
        this(file, filters, GenericData.get());
    }

    public DocDataFileReader(Input data, Input head, FilterOperator[] filters) throws IOException {
        this(data, head, filters, GenericData.get());
    }

    public DocDataFileReader(File file, FilterOperator[] filters, GenericData model) throws IOException {
    ...
    public DocDataFileReader(Input data, Input head, FilterOperator[] filters, GenericData model)
            throws IOException {
...
//        }
        ...
        this.skipTable = new SkipTree(readColumns, values, this.dir);
...
//        System.out.println(this.readParent);
    ...
    public void tranBitset(String targetField) throws IOException {
        ...
        filterSet = this.skipTable.bitSetTranShallowFirst(filterSet, currentColumnIndex, tm);
        if(filterSetMap.containsKey(tm)){
            filterSet.and(filterSetMap.get(tm));
        }
        filterSetMap.put(tm, (BitSet) filterSet.clone());
...
//        long start = System.currentTimeMillis();
        ...
        readLength.put(readParent, Math.min(all, max));
        for (int i = 0; i < readNO.length; i++) {
